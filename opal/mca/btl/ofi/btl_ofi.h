...
#if 0
static inline bool mca_btl_ofi_context_trylock (mca_btl_ofi_device_context_t *context)
{
    return (context->lock || OPAL_ATOMIC_SWAP_32 (&context->lock, 1));
}

static inline void mca_btl_ofi_context_lock (mca_btl_ofi_device_context_t *context)
{
    while (mca_btl_ofi_context_trylock (context));
}

static inline void mca_btl_ofi_context_unlock (mca_btl_ofi_device_context_t *context)
{
    opal_atomic_mb ();
    context->lock = 0;
}
