...
int sync_wait_mt(ompi_wait_sync_t *sync)
{
    ...
    if( NULL == wait_sync_list ) {
        ...
        wait_sync_list = sync;
        sync->wakeup = 1;
        //opal_output(0, "sync %p arrived to see empty queue, taking progress",sync);
    } else {
 ...
 check_status:
    if( sync != wait_sync_list ) {
        ...
        while(!sync->wakeup){
            nanosleep(&spec, NULL);
            if(sync->count == 0 || sync == wait_sync_list ) {
                //opal_output(0,"sync %p wake up sync->count = %d, next progress = %d",sync->count,(sync == wait_sync_list) ? 1 : 0);
                break;
            }
	    }
        ...
        if( sync->count <= 0 ) {  /* Completed? */
            //opal_output(0,"sync %p finished without working, getting out",sync);
            goto i_am_done;
        }
        /* either promoted, or spurious wakeup ! */
        //opal_output(0,"sync %p promoted to take progress",sync);
        goto check_status;
    }

    //opal_output(0,"sync %p take progress",sync);
    while(sync->count > 0) {  /* progress till completion */
        opal_progress();  /* don't progress with the sync lock locked or you'll deadlock */
    }
    //opal_output(0,"sync %p getting out",sync);
 ...
 i_am_done:
    ...
    if( sync == wait_sync_list ) {
        //opal_output(0,"sync %p passing progress to the  next sync -> %p",sync,(sync == sync->next) ? NULL : sync->next);
        wait_sync_list = (sync == sync->next) ? NULL : sync->next;
        if( NULL != wait_sync_list ){
            //opal_output(0,"sync %p signaled",wait_sync_list);
